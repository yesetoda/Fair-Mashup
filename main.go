...
func problemsHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Using a worker pool to limit concurrency and fetch solved/tried problems
...
// Worker pool implementation to limit concurrent API calls
func workerPoolGetSolvedAndTriedProblems(participants []string) (map[string]bool, map[string]bool) {
	...
	sem := make(chan struct{}, maxWorkers) // Limit the number of concurrent workers
	...
	for _, handle := range participants {
		...
		go func(handle string) {
			...
			sem <- struct{}{} // Block if max workers are busy
			defer func() { <-sem }()
